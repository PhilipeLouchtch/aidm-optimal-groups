...
calc_aupcr <- function(num_projs, raw_profile) {
    ...
    # The above cumsum of a profile is not enough for AUPC, need to add the last sum up to the highest rank (#projects)
    # Example:
    # Let "3|2|1" be a profile of 6 students in a setting with 10 projects, then cumsum will give us [3, 5, 6]
    # but we need the sum of [3, 5, 6, 6, 6, 6, 6, 6, 6, 6], so we add 6 * (10-3) to the sum of the cumsum
