...
prepare_data <- function(data, editions) {
    ...
    data_longer <- data |>
        ...
        mutate(
            r_aupcr = map2( worst_rank__grp, profile, ~ ifelse(is.na(.y), NA, calc_aupcr(.x, .y)) ) |> unlist() #
        )
...
prepare_data_satunsatcombined <- function(data, editions) {
    ...
    data_longer <- data |>
        ...
        # if the grouping is empty (then set AUPCR to 1 - this only makes sense for unsat group)
        mutate(
            r_aupcr = map2( worst_rank__grp, profile_combined, ~ ifelse(is.na(.y), NA, calc_aupcr(.x, .y)) ) |> unlist() #
        )
...
for (scale_factor in scale_factors) {
    ...
    print(paste("Base:     ", calc_aupcr(worst,  profile)))
    print(paste("Scaled   :", calc_aupcr(worst,  scaled_profile_serialized)))
    ...
    print(paste("Base-norm:", calc_aupcr(worst, profile_norm_ser)))
    print(paste("Scaled-nm:", calc_aupcr(worst, scaled_profile_add_norm_ser)))
