...
qweqwe <- historical_data_tu |>
                ...
                mutate(
                    ...
                    aupcr = pmap(list(num_projects, rank), calc_aupcr) |> unlist(),
    ...
    data |>
        ...
        mutate(
            ...
            student_type = factor(student_type,
                                  levels = c("all", "solo", "pregrouped", "unsatisfied"),
                                  labels = c("all", "solo", "pregroup", "unsat"))
        )
}
...
plot_historical_bepsys_vs_chia <- function(title, data) {
    data_processed <- transform_hist_data(data) |> unchop(rank)
    
    plot <- ggplot(data_processed,
                aes(x = interaction(student_type, mechanism_short), 
                    y = rank,
                    group = interaction(student_type, mechanism_short),
        ...
        scale_x_discrete(guide = guide_axis_nested(inv = F, angle = 0 )) +
        ...
        theme(legend.key.size = unit(5, "mm"), legend.position = "none")
...
plot_bepsys_chia_max_selected <- plot_historical_bepsys_vs_chia(
    "",
...
print(plot_bepsys_chia_max_selected)
ggsave(plot = plot_bepsys_chia_max_selected,
       filename = "plots/thesis/hist_bepsys_vs_chia_max_selected.png",
