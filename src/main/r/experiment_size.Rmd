...
data_exps <- loadSizeExperimentData(exp_name)

# preprocess
data_exps_aug <- data_exps |> augment_with_short_mechanism_name()
...
for (ppt in levels(data_exps_aug$proj_pref_type)){
    # for (mech in levels(data_exps_aug$mechanism)) {
        ...
        plot <- ggplot(data_filtered, aes(x = num_students, y = aupcr, group = interaction(num_students, mechanism), fill = mechanism, color = mechanism)) +
                    geom_boxplot() +
                    # geom_jitter(position=position_jitter(0.2)) +
                    facet_grid(~num_projects, space = "free", scales = "free_x", shrink = TRUE) +
                    # ylim(0.5, 1) + 
                    # coord_flip() + 
                    scale_y_continuous() +
                    scale_x_continuous(breaks = data_filtered$num_students |> unique() |> sort(), guide = guide_axis(angle = 90)) +
                    labs(
