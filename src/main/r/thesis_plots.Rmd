...
data_gsb_q <- loadGroupSizeBoundsExperimentData("gsc_exp_final1") |>
    # exclude not interesting results and keep only trial 1 runs
...
plot_gsb_q <- function(num_studs) {
    ...
    plot <- ggplot(data_aug,
               aes(x = interaction(projectpref_short, mechanism_short),
                   group = interaction(projectpref_short, mechanism_short),
                   )
            ) +
            ...
            geom_boxplot(aes(y = worst_rank_scaled),
                         stroke = 0.2,
                         color = "black",
                         shape = 8, size = 1, position = position_dodge(width = 0.7)
            ) +
            scale_y_continuous(sec.axis = sec_axis(name = "Worst rank", ~ . * worst)) +
...
ggsave(plot = plot_gsb_q(200),
       filename = paste0("plots/thesis/synth_gsb_s", 200, ".png"),
       device = "png", width = 330, height = 206, units = "mm", dpi = "print")
       
ggsave(plot = plot_gsb_q(600),
       filename = paste0("plots/thesis/synth_gsb_s", 600, ".png"),
       device = "png", width = 330, height = 206, units = "mm", dpi = "print")
```
