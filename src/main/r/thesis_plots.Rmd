...
plot <- ggplot(data_slots,
               aes(x = mechanism_short,
                   y = duration_ms / 1000,
                   group = interaction(mechanism_short, num_slots),
                   color = num_slots)
               ) +
        geom_boxplot() + 

        facet_grid(num_students + project_pressure ~ projectpref_short,
                   scale = "free_y",
                   labeller = partial(label_value, multi_line = FALSE)
                   ) +
...
for (pref_scenario in unique(data_groupsizb$proj_pref_type)) {
    
    plot <- ggplot(data_groupsizb %>%
                       filter(num_students != 50) %>%
                       filter(proj_pref_type == pref_scenario),
