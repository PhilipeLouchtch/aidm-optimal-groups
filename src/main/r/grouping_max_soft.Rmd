...
# These are some trial and error plotting experiments with generated data
...
filename <- paste(path, paste0("synth_group_maxsoft_runtimes", ".png"), sep ="/")
ggsave(filename,
       device = "png", width = 230, height = 80, units = "mm", dpi = 300, pointsize = 12
)
...
# Thesis: HISTO PLOT, upper/lower - Quality comparison between types (Only Chia vs Fair, no 100%)
```{r, fig.height = 15}
# unique(data_all$mechanism)
for (studs in levels(ordered(unique(data_pregroup_soft$num_students)))) {
    ppt <- "realistic"
