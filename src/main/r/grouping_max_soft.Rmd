...
data_pregroup_soft_f <- data_pregroup_soft %>%
    ...
    filter(mechanism_short != 'BEPSys' & gd == 'MAX_ONLY')
...
# Thesis (mentioned, not included): runtimes, deep-dive tight pressure results of Fair, 600, rnd
```{r, fig.height=4, fig.width=11, warning=FALSE}
data_pregroup_soft_f <- data_pregroup_soft %>%
    filter(trial == 0) |>
    mutate(
    ...
    filter(gd == 'MAX_ONLY') |>
    filter(mechanism_short == 'Fair' & num_students == 600 & proj_pref_type == 'rnd')

print(data_pregroup_soft_f)
...
#    "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00"
    ...
    # facet_grid(num_students ~ project_pressure) +
    facet_wrap(vars(pregroup_type, ), nrow=1) + 
